...
function checkCanModifyAfterTurn(gameState: GameState): boolean {
  ...
  const modificationRules = allRules.filter(rule =>
...
export const processTileModification = (
  ...
  switch (modification.type) {
    case 'add':
      ...
      if (newState.boardConfig?.maxTiles && newState.tiles.length >= newState.boardConfig.maxTiles) {
        return { newState: gameState, success: false, message: 'Maximum tiles reached' };
      }

      // Check if position is already occupied
      const existingTile = newState.tiles.find(
        t => t.position.x === modification.position!.x && t.position.y === modification.position!.y
      );
      if (existingTile) {
    ...
    case 'remove':
      ...
      if (newState.boardConfig?.minTiles && newState.tiles.length <= newState.boardConfig.minTiles) {
      ...
      if (playersOnTile.length > 0) {
        return { newState: gameState, success: false, message: 'Cannot remove tile with players on it' };
      }

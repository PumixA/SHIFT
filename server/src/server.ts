...
io.on('connection', (socket) => {
    ...
    socket.on('update_user_profile', async (data: { userId: string; username?: string; avatarUrl?: string; avatarPreset?: string }) => {
    ...
    socket.on('invite_to_game', async (data: { userId: string; friendId: string; roomId: string; roomName?: string }) => {
        const targetSocketId = friendService.getUserSocketId(data.friendId);
    ...
    socket.on('chat_message', async (data: { roomId: string; content: string; senderName: string }) => {
    ...
    socket.on('chat_emoji_reaction', (data: { roomId: string; emoji: string; senderName: string }) => {
    ...
    socket.on('get_game_history', async (data: { userId: string; limit?: number; offset?: number }) => {
    ...
    socket.on('register_push_subscription', async (data: { userId: string; subscription: any }) => {
        const result = await pushNotificationService.registerSubscription(data.userId, data.subscription);
    ...
    socket.on('effect_applied', (data: { roomId: string; playerId: string; effectType: string; duration: number }) => {
    ...
    socket.on('create_room', async (data: {
        ...
        const { roomId, roomName, password, maxPlayers, allowRuleEdit, allowTileEdit, rulePackId, playerName } = data;
        ...
        console.log(`[Room] Created with settings: maxPlayers=${maxPlayers}, allowRuleEdit=${allowRuleEdit}`);
    });
    ...
    socket.on('join_room', async (roomId: string, playerData?: { name?: string; userId?: string; avatarPreset?: string }) => {
        socket.rooms.forEach(room => {
            if (room !== socket.id) socket.leave(room);
        });
    ...
    games[roomId] = createInitialGameState(roomId, `Room ${roomId.substring(0, 6)}`);
        ...
        if (game.players.length < 4) {
            ...
            if (!existingPlayer) {
                const colors: ('cyan' | 'violet' | 'orange' | 'green')[] = ['cyan', 'violet', 'orange', 'green'];
                ...
                // System message
                const systemMsg = chatService.createSystemMessage(roomId, `${newPlayer.name} joined the game`);
                io.to(roomId).emit('chat_message', systemMsg);
            }
        } else {
    ...
    socket.on('modify_rule', (data: { type: 'add' | 'modify' | 'delete'; ruleId?: string; rule?: any }) => {
        const roomId = getRoomIdBySocket(socket);
        ...
        if (result.success) {
            ...
            const actionType = data.type === 'add' ? 'rule_added' :
                               data.type === 'modify' ? 'rule_modified' : 'rule_deleted';
    ...
    socket.on('request_rematch', async (data: { roomId: string }) => {
        ...
        if (game.winnerId) {
            const duration = game.startedAt ? Math.floor((Date.now() - new Date(game.startedAt).getTime()) / 1000) : 0;
    ...
    socket.on('kick_player', (data: { roomId: string; playerId: string }) => {
        ...
        // System message
        const systemMsg = chatService.createSystemMessage(data.roomId, `${playerName} was removed from the game`);
        io.to(data.roomId).emit('chat_message', systemMsg);
    ...
    socket.on('update_game_settings', (data: {
        ...
        // System message
        const systemMsg = chatService.createSystemMessage(data.roomId, 'Game settings have been updated');
        io.to(data.roomId).emit('chat_message', systemMsg);
        ...
        if (game.status === 'finished') {
            ...
            if (player.extraTurns && player.extraTurns > 0) {
                ...
                if (nextPlayer.userId) {
                    await pushNotificationService.sendYourTurnNotification(
                        nextPlayer.userId,
                        game.roomName
                    );
                }
    ...
    socket.on('save_rule_pack', async (data: { name: string; description?: string; isPublic?: boolean }) => {
        const roomId = getRoomIdBySocket(socket);
        if (!roomId || !games[roomId]) {
        ...
        const game = games[roomId];
        const pack = await rulePackService.createRulePack(
            data.name,
            game.activeRules,
            {

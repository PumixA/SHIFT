...
interface GamepadState {
    ...
    id: string
    connected: boolean
    mapping: ExtendedGamepadMapping
...
export function useMultiGamepad(options: UseMultiGamepadOptions) {
    const { assignments, onAction, pollInterval = 16, deadzone = 0.15, vibrationEnabled = true } = options
    ...
    const vibrateForPlayer = useCallback(
        (playerId: string, duration: number = 200, intensity: number = 1.0) => {
    ...
    const vibrateAll = useCallback(
        (duration: number = 200, intensity: number = 1.0) => {
            if (!vibrationEnabled) return

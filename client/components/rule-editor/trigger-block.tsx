...
import { TriggerType, TRIGGER_INFO } from "@/src/types/rules"
...
export function TriggerBlock({ trigger, onChange, onSelectTile }: TriggerBlockProps) {
  ...
  const groupedTriggers = Object.entries(TRIGGER_INFO).reduce((acc, [type, info]) => {
    const category = info.category
    if (!acc[category]) acc[category] = []
    acc[category].push({ type, ...info })
    return acc
  }, {} as Record<string, typeof TRIGGER_INFO[TriggerType][]>)
  ...
  return (
    <div className="space-y-4">
      ...
      {needsValue && (
        <div className="space-y-2">
          <Label>
            {valueType === 'tile' ? 'Case cible' :
             valueType === 'dice' ? 'Valeur du dé' :
             valueType === 'score' ? 'Seuil de score' :
             valueType === 'turns' ? 'Nombre de tours' :
             'Valeur'}
          </Label>
          <div className="flex gap-2">
            <Input
              ...
              placeholder={
                valueType === 'tile' ? 'Index de la case' :
                valueType === 'dice' ? '1-6' :
                '0'
              }
            />

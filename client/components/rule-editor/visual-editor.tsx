...
export function VisualEditor({ initialRule, onSave, onCancel, onSelectTile }: VisualEditorProps) {
    ...
    const addCondition = () => {
        setConditions([...conditions, { type: ConditionType.SCORE_CHECK, operator: "gte", value: 0, target: "self" }])
    }
    ...
    const addEffect = () => {
        setEffects([...effects, { type: ActionType.MOVE_RELATIVE, value: 1, target: "self" }])
    }
    ...
    const moveEffect = (index: number, direction: "up" | "down") => {
        if ((direction === "up" && index === 0) || (direction === "down" && index === effects.length - 1)) {
            return
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <Card>
                ...
                <CardContent>
                    <TriggerBlock trigger={trigger} onChange={setTrigger} onSelectTile={onSelectTile} />
                </CardContent>
            </Card>
                    ...
                    <div className="space-y-3">
                        {effects.map((effect, index) => (
                            <div key={index} className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                    <EffectBlock effect={effect} onChange={(e) => updateEffect(index, e)} />
                                </div>
                                <Button
                                    size="icon"
                                    variant="ghost"

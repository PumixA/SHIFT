...
import { Shield, Zap, Clock, Snail, Ghost, Swords, Dice1, Dice6, TrendingUp } from "lucide-react"
                        ...
                        return (
                            <Tooltip key={effect.id}>
                                <TooltipTrigger asChild>
                                    <div
                                        className={`flex h-6 w-6 items-center justify-center rounded-full border ${config.color}`}
                                    >
                                ...
                                <TooltipContent>
                                    ...
                                    <p className="mt-1 text-xs">
                                        ...
                                        {effect.turnsRemaining} tour{effect.turnsRemaining > 1 ? "s" : ""} restant
    ...
    return (
        <div className="space-y-2">
            {effects.map((effect) => {
                ...
                return (
                    <div key={effect.id} className={`flex items-center gap-3 rounded-lg border p-2 ${config.color}`}>
                        <div className="bg-background/50 flex h-8 w-8 items-center justify-center rounded-full">
    ...
    return (
        <div className="mt-1 flex flex-wrap gap-1">
            {effects.slice(0, 3).map((effect) => {
                ...
                return (
                    <Badge key={effect.id} variant="secondary" className={`h-5 px-1.5 text-[10px] ${config.color}`}>

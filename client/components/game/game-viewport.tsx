...
export const GameViewport = forwardRef<GameViewportRef, GameViewportProps>(
  ({ tiles, players, currentTurn, onAddTile, onCenterCamera, isSelectionMode, onTileClick, rules = [], onTileDetails, isRemoveTileMode, onRemoveTile, canModifyTiles = true }, ref) => {
    ...
    const getRulesForTile = useCallback((tileIndex: number) => {
      return rules.filter(rule => {
        ...
        if (triggerType !== 'ON_LAND' && triggerType !== 'ON_PASS_OVER' && triggerType !== 'ON_REACH_POSITION') {
          return false
        }
    ...
    const isTileAt = useCallback((x: number, y: number) => {
      return tiles.some(t => t.x === x && t.y === y)
    }, [tiles])

    // Obtenir les directions disponibles pour ajouter une case
    const getAvailableDirections = useCallback((tileX: number, tileY: number) => {
    ...
    return (
      <div className={`absolute inset-0 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 ${isSelectionMode ? 'cursor-crosshair' : ''}`}>
        {/* Grille de fond */}
        <div
                  ...
                  <ContextMenuTrigger asChild>
                    <div style={{ position: 'absolute', left: worldPos.x, top: worldPos.y }}>
                      <GameTile
                        ...
                        rules={tileRules}
                        showExpandButtons={canModifyTiles && selectedTileForExpand?.x === tile.x && selectedTileForExpand?.y === tile.y}
                    ...
                    {canModifyTiles && (
                      <>
                        ...
                        {availableDirections.map(({ dir }) => (
                          <ContextMenuItem key={dir} onClick={() => handleAddTileFromTile(tile, dir)}>
                            <Plus className="h-4 w-4 mr-2" />
                            Ajouter case {dir === "up" ? "en haut" : dir === "down" ? "en bas" : dir === "left" ? "à gauche" : "à droite"}
                          </ContextMenuItem>
                        ))}
        ...
        {/* Instructions */}
        <div className="hidden md:flex absolute bottom-4 left-4 items-center gap-2 text-xs text-muted-foreground z-10 bg-black/50 px-3 py-2 rounded-lg">
...
function GameTile({
  ...
  const hasPositiveRules = rules.some(r =>
    r.effects.some(e => {
      ...
      return (e.type === 'MOVE_RELATIVE' && val > 0) ||
  ...
  return (
    <div className="relative">
      <div
        onClick={onClick}
        className={`w-16 h-16 rounded-xl border-2 flex items-center justify-center font-bold text-lg transition-all duration-300 ${getStyles()} ${getModeClass()}`}
      >
        {getLabel()}
        ...
        {hasRules && tile.type !== 'start' && tile.type !== 'end' && (
          <div className="absolute inset-0 flex items-center justify-center">
            <Zap className={`h-6 w-6 drop-shadow-lg ${hasNegativeRules && !hasPositiveRules ? 'text-red-300' : hasPositiveRules && !hasNegativeRules ? 'text-green-300' : 'text-yellow-300'}`} />
          </div>
        )}
        ...
        {hasRules && (
          <Badge
            variant="secondary"
            className={`absolute -top-2 -right-2 h-5 w-5 p-0 flex items-center justify-center text-[10px] font-bold shadow-lg ${
        ...
        {/* Icône info */}
        {hasRules && (
          <Info className="absolute bottom-1 right-1 h-3 w-3 opacity-60" />
        )}

        {/* Index */}
        <span className="absolute bottom-0.5 left-1 text-[9px] opacity-50 font-mono font-bold">
          {index}
        </span>

        {/* Mode suppression */}
        {isRemoveTileMode && tile.type !== 'start' && tile.type !== 'end' && (
          <div className="absolute inset-0 bg-red-500/30 rounded-xl flex items-center justify-center backdrop-blur-sm">
        ...
        <>
          {availableDirections.map(({ dir }) => (
            <Button
              ...
              size="icon"
              variant="outline"
              className={`absolute h-6 w-6 bg-cyan-500/20 border-cyan-400/50 hover:bg-cyan-500/40 text-cyan-400 ${
                dir === 'up' ? '-top-8 left-1/2 -translate-x-1/2' :
                dir === 'down' ? '-bottom-8 left-1/2 -translate-x-1/2' :
                dir === 'left' ? 'top-1/2 -left-8 -translate-y-1/2' :
...
function PlayerToken({
  ...
  const colors = {
    cyan: { bg: "bg-cyan-400", border: "border-cyan-300", glow: "shadow-[0_0_15px_rgba(34,211,238,0.8)]", text: "text-cyan-900" },
    violet: { bg: "bg-violet-400", border: "border-violet-300", glow: "shadow-[0_0_15px_rgba(168,85,247,0.8)]", text: "text-violet-900" },
    orange: { bg: "bg-orange-400", border: "border-orange-300", glow: "shadow-[0_0_15px_rgba(251,146,60,0.8)]", text: "text-orange-900" },
    green: { bg: "bg-green-400", border: "border-green-300", glow: "shadow-[0_0_15px_rgba(74,222,128,0.8)]", text: "text-green-900" },
  }

  const colorStyle = colors[player.color] || colors.cyan
  ...
  return (
    <div
      className={`absolute w-7 h-7 rounded-full border-3 transition-all duration-300 ${colorStyle.bg} ${colorStyle.border} ${isActive ? colorStyle.glow : ""} ${isActive ? "scale-125 z-20" : "scale-100 z-10"}`}
      style={{
        left: x + TILE_SIZE / 2 - 14 + offsetX,
        top: y + TILE_SIZE / 2 - 14 + offsetY,
      }}
    >
      <span className={`absolute inset-0 flex items-center justify-center text-[11px] font-black ${colorStyle.text}`}>
        {player.name.charAt(0).toUpperCase()}
      </span>
      {isActive && (
        <span className="absolute -top-1 -right-1 h-3 w-3 bg-yellow-400 rounded-full animate-pulse border border-yellow-300" />
      )}
    </div>
  )
}

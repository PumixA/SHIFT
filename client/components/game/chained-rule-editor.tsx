...
export function ChainedRuleEditor({
  initialRule,
  onSave,
  onCancel,
  existingRules = [],
}: ChainedRuleEditorProps) {
  ...
  const [showElse, setShowElse] = useState(
    (initialRule?.elseActions?.length || 0) > 0
  )
  ...
  const addCondition = useCallback((templateId?: string) => {
    const template = templateId
      ? CONDITION_TEMPLATES.find(t => t.id === templateId)
      : CONDITION_TEMPLATES[0]
  ...
  const addAction = useCallback((type: "then" | "else", templateId?: string) => {
    const template = templateId
      ? ACTION_TEMPLATES.find(t => t.id === templateId)
      : ACTION_TEMPLATES[0]
  ...
  return (
    <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              <span className="text-yellow-400 font-black">SI</span>
      ...
      <Card className="border-green-500/30 bg-green-500/5">
        <CardHeader
          className="cursor-pointer"
          onClick={() => toggleSection("then")}
        >
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              <span className="text-green-400 font-black">ALORS</span>
                ...
                <div className="space-y-2">
                  {rule.thenActions?.map((action) => (
                    ...
                    >
                      ...
                      <Select
                        value={action.type}
                        onValueChange={(v) => updateAction("then", action.id, {
                      ...
                      {action.type !== ActionType.EXTRA_TURN &&
                       ...
                       action.type !== ActionType.BACK_TO_START && (
                        <Input
                          ...
                          onChange={(e) => updateAction("then", action.id, {
      ...
      <div className="flex items-center justify-between p-4 rounded-lg bg-white/5 border border-white/10">
        <div className="flex items-center gap-3">
          <span className="text-red-400 font-black">SINON</span>
        ...
        {showElse && (
          ...
          >
            <Card className="border-red-500/30 bg-red-500/5">
              <CardHeader
                className="cursor-pointer"
                onClick={() => toggleSection("else")}
              >
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                    <span className="text-red-400 font-black">SINON</span>
                    <Badge variant="outline" className="text-xs text-red-400">
                      {rule.elseActions?.length || 0} action{(rule.elseActions?.length || 0) !== 1 ? 's' : ''}
                    </Badge>
                  </CardTitle>
                      ...
                      <div className="space-y-2">
                        {rule.elseActions?.map((action) => (
                          ...
                          >
                            ...
                            <Select
                              value={action.type}
                              onValueChange={(v) => updateAction("else", action.id, {
                                type: v as ActionType,
                              })}
                            >
                              <SelectTrigger className="flex-1 h-8">
                            ...
                            {action.type !== ActionType.EXTRA_TURN &&
                             ...
                             action.type !== ActionType.BACK_TO_START && (
                              <Input
                                type="number"
                                value={action.value || 0}
                                onChange={(e) => updateAction("else", action.id, {
                                  value: Number(e.target.value),
                                })}
                                className="w-20 h-8"
                              />
      ...
      {errors.length > 0 && (
        <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30">
          <p className="text-sm font-bold text-red-400 mb-2">Erreurs :</p>

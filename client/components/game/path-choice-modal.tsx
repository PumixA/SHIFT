...
export function PathChoiceModal({
  ...
  const displayedPath = hoveredPathIndex !== null
        ...
        <div className="grid gap-4">
                ...
                {tiles.map((tile) => {
                  ...
                  return (
                    <div
                      key={tile.id}
                      className={cn(
                        "absolute w-8 h-8 rounded-md border-2 transition-all duration-200",
                        "flex items-center justify-center text-[10px] font-bold",
                        isDestination
                          ? `${colors.border} ${colors.bg}/40 scale-110 z-20`
                          : isStart
                          ? "border-white/60 bg-white/20 z-10"
                          : isInPath
                    ...
                    >
                      {isDestination && <MapPin className={`h-4 w-4 ${colors.text}`} />}
                      {isStart && !isDestination && <div className={`w-2 h-2 rounded-full ${colors.bg}`} />}
                    ...
                    {displayedPath.path.slice(0, -1).map((step, idx) => {
                      ...
                      return (
                        <line
                          ...
                          x2={x2}
                          y2={y2}
                          stroke={playerColor === "cyan" ? "#22d3ee" :
                                  playerColor === "violet" ? "#a78bfa" :
                                  playerColor === "orange" ? "#fb923c" : "#4ade80"}
                          strokeWidth="3"
                          strokeLinecap="round"
                          strokeDasharray="6 4"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              ...
              className={cn(
                "gap-2",
                selectedPathIndex !== null && `${colors.bg} hover:opacity-90`
              )}
            >

...
function RuleAnimation({ event, onComplete }: { event: RuleAnimationEvent; onComplete: () => void }) {
    ...
    return (
        ...
        >
            ...
            >
                {/* Icon with glow */}
                <motion.div
                    className={cn("relative flex h-24 w-24 items-center justify-center rounded-full", config.bgColor)}
                    animate={{
                        scale: [1, 1.2, 1],
                        boxShadow: [
                ...
                {particles.map((particle) => (
                    <motion.div
                        key={particle.id}
                        className={cn("absolute h-2 w-2 rounded-full", config.bgColor)}
                        style={{
                            top: "50%",
                            left: "50%",
                        }}
                ...
                >
                    <motion.h3
                        className={cn("text-2xl font-black tracking-wider uppercase", config.color)}
                ...
                {event.details ? (
                    ...
                    >
                        {event.details.scoreChange !== undefined && (
                            ...
                            >
                                {event.details.scoreChange > 0 ? "+" : ""}
                                {event.details.scoreChange} points
                            </motion.div>
                        )}
                        {event.details.movement !== undefined && (
                            <motion.div
                                className="rounded-full bg-blue-500/20 px-4 py-2 text-lg font-bold text-blue-400"
                                animate={{ x: [0, 5, 0] }}
                                transition={{ duration: 0.3, repeat: 3 }}
                            >
                                {event.details.movement > 0 ? "+" : ""}
                                {event.details.movement} cases
                            </motion.div>
                        )}
                        {event.details.turnsRemaining !== undefined && (
                            <div className="rounded-full bg-violet-500/20 px-4 py-2 text-lg font-bold text-violet-400">
...
function MiniRuleAnimation({ event, onComplete }: { event: RuleAnimationEvent; onComplete: () => void }) {
    const config = ANIMATION_CONFIGS[event.actionType] || ANIMATION_CONFIGS.DEFAULT
    const Icon = config.icon
    ...
    return (
        ...
        >
            ...
            <div>
                <p className={cn("text-sm font-bold", config.color)}>{event.ruleName}</p>
        ...
        <>
            ...
            <AnimatePresence>
                {currentAnimation && animationMode === "full" ? (
                    <RuleAnimation event={currentAnimation} onComplete={handleAnimationComplete} />
                ) : null}
            </AnimatePresence>

            {/* Mini toast animations */}
            <div className={cn("fixed top-4 right-4 z-50 flex w-80 flex-col gap-2", className)}>

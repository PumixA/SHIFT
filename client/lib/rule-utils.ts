...
export function getRuleDescription(rule: Rule): string {
    ...
    switch (effect.type) {
        case ActionType.MOVE_RELATIVE:
            actionDesc =
                Number(effect.value) > 0 ? `avancer de ${effect.value}` : `reculer de ${Math.abs(Number(effect.value))}`
            break
        case ActionType.MODIFY_SCORE:
            actionDesc =
                Number(effect.value) > 0 ? `gagner ${effect.value} pts` : `perdre ${Math.abs(Number(effect.value))} pts`
            break

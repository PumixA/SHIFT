...
export function evaluateCondition(condition: RuleCondition, context: EvaluationContext): boolean {
    ...
    switch (condition.type) {
        ...
        case "TILE_TYPE":
            const currentTile = context.tiles.find((t) => t.x === getTargetPosition(target || "self", context))
    ...
    switch (target) {
        ...
        case "nearest_player":
            ...
            return context.players
                ...
                .reduce((a, b) => (Math.abs(a.position - selfPos) < Math.abs(b.position - selfPos) ? a : b))
        default:
            return context.players.find((p) => p.id === context.currentPlayerId)
    }
}
...
function getNearestPlayerPosition(context: EvaluationContext): number {
    ...
    return others.reduce((a, b) => (Math.abs(a.position - selfPos) < Math.abs(b.position - selfPos) ? a : b)).position
}
...
export function convertToChainedRule(rule: Rule): ChainedRule {
    ...
    // Get first effect from effects array
    ...
    return {
        ...
        triggerTileId: rule.trigger.value?.toString(),
        conditions,
        conditionLogic: "AND",
        thenActions: firstEffect
            ? [
                  {
                      id: `action-${rule.id}`,
                      type: firstEffect.type as ActionType,
                      value: typeof firstEffect.value === "number" ? firstEffect.value : undefined,
                      targetPlayer: (firstEffect.target as ConditionTarget) || "self",

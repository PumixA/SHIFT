...
export class BotAI {
     ...
     */
    async decideAction(bot: BotPlayer, gameState: GameState, canModify: boolean): Promise<BotDecision> {
    ...
    private isTileDangerous(tile: GameTile, rules: Rule[]): boolean {
        ...
        const applicableRules = rules.filter(
            (r) => r.trigger.type === TriggerType.ON_LAND || r.trigger.type === TriggerType.ON_PASS_OVER
        )

        for (const rule of applicableRules) {
            const triggerTileId = rule.trigger.value
            if (triggerTileId === tile.id || !triggerTileId) {
                // Check if action is harmful (use first effect)
                ...
                if (harmfulActions.includes(effect.type as ActionType)) {
                    if (effect.type === ActionType.MODIFY_SCORE && (Number(effect.value) || 0) < 0) {
                        return true
                    }
                    if (effect.type === ActionType.MOVE_RELATIVE && (Number(effect.value) || 0) < 0) {
                        return true
                    }
                    if ([ActionType.BACK_TO_START, ActionType.SKIP_TURN].includes(effect.type as ActionType)) {
                        return true
                    }
                }
            }
    ...
    private isTileBeneficial(tile: GameTile, rules: Rule[]): boolean {
        ...
        for (const rule of applicableRules) {
            ...
            if (triggerTileId === tile.id || !triggerTileId) {
                ...
                if (beneficialActions.includes(effect.type as ActionType)) {
                    return true
                }
            }
        }
    ...
    private analyzeOpponents(bot: BotPlayer, opponents: PlayerState[], tiles: GameTile[]): OpponentAnalysis {
        ...
        const nearestToEnd = opponents.reduce((a, b) => (tiles.length - a.position < tiles.length - b.position ? a : b))
     ...
     */
    private decideModification(bot: BotPlayer, gameState: GameState, analysis: GameAnalysis): BotDecision | null {
     ...
     */
    private createStrategicRule(bot: BotPlayer, gameState: GameState, analysis: GameAnalysis): BotDecision {
        ...
        if (!analysis.isWinning && analysis.opponentAnalysis) {
            return {
                ...
                details: {
                    ...
                    conditions: [
                        { type: "SCORE_CHECK", operator: "lt", value: analysis.opponentAnalysis.averageScore },
                    ],
...
export function getBotThinkingMessage(difficulty: BotDifficulty): string {
    const messages = {
        easy: ["Hmm, voyons voir...", "Euh... que faire ?", "Je réfléchis...", "Bon, allez !"],
        medium: ["Analysons la situation...", "Intéressant...", "Je calcule les options...", "Stratégie en cours..."],
        hard: [
            "Analyse approfondie...",
